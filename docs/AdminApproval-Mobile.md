...
بعد از یک‌بار تأیید، تا وقتی ویرایش نشود دوباره نیاز به تأیید قالب ندارد.

**ارسال آنی (مورد ۱۳):** اگر پیام بر اساس قالب `Approved` باشد (مثلاً قالب پیش‌فرض تأییدشده)، SMS **فوری** ارسال می‌شود و به صف تأیید پیام (`message-approvals`) نمی‌رود. با ویرایش نام/محتوای قالب، وضعیت دوباره `Pending` می‌شود و تا تأیید بعدی ارسال با آن قالب مسدود/صف می‌شود.

تنظیم پیش‌فرض (`set-default`) فقط برای قالب `Approved` مجاز است.
...
| BusinessCard / UserForm / LuckyWheel / BookingSystem quick-send | `POST /api/{Module}/quick-send` |
| تأیید کمپین | `POST /api/Message/campaign/{id}/confirm-and-send` |
| QuickAction | `.../quick-send` |

اگر پیام از قالب تأییدشده استفاده کند (`templateId` یا متن دقیقاً برابر قالب Approved):
...
| وضعیت | نمونه `message` |
|-------|------------------|
| ارسال آنی با قالب تأییدشده | پاسخ `200` (ارسال انجام شد) |
...
| تنظیم پیش‌فرض بدون تأیید | `فقط قالب تأییدشده می‌تواند به‌عنوان پیش‌فرض تنظیم شود` |
| ارسال سریع Pending | `پیام سریع شما در صف تأیید ادمین است. پس از تأیید، می‌توانید بدون نیاز به تأیید مجدد ارسال کنید.` |
| ارسال سریع Rejected | `محتوای ارسال سریع … رد شده است. دلیل: …` |

---

## ۵) ارسال سریع ماژول‌ها (کارت ویزیت / رزرو / فرم / گردونه / لینک / اقدام)

### رفتار بک‌اند (مثل قالب — یک‌بار تأیید روی خود آیتم)

| رویداد | `approvalStatus` |
|--------|------------------|
| ساخت / انتشار / ویرایش محتوا | `Pending` |
| تأیید ادمین | `Approved` — ارسال‌های بعدی بدون صف پیام |
| رد ادمین | `Rejected` (+ `rejectionReason`) |
| ویرایش محتوا بعد از تأیید | دوباره `Pending` |
| فقط تغییر `IsActive` | تأیید باطل نمی‌شود |

Admin API: `/api/Admin/QuickSendApproval`  
`itemType`: `BusinessCard` | `BookingSystem` | `UserForm` | `LuckyWheel` | `SocialMediaLink` | `QuickAction`
...
- فیلد اختیاری `smsDescription` (توضیحات ارسال، حداکثر ۱۰۰ کاراکتر)
- متن SMS واقعی: `{smsDescription}\n{publicUrl یا linkUrl}` یا فقط URL اگر توضیحات خالی باشد
...
2. badge در لیست هر ماژول (نیازی به صفحه جدا نیست)
3. quick-send: اگر `statusCode == 202` → toast متن `message` بک‌اند (صف تأیید یک‌باره)
4. اگر `Rejected` → نمایش دلیل رد
5. رزرو نوبت: endpoint `POST /api/BookingSystem/quick-send` را هم اضافه کنید
...
2. **UX موبایل لازم است** تا کاربر فکر نکند پیام ارسال شده وقتی هنوز Pending است (`statusCode == 202`).
3. قالب = یک‌بار تأیید؛ ویرایش نام/محتوا = دوباره Pending.
4. ارسال با قالب `Approved` = آنی (بدون صف تأیید پیام)؛ متن آزاد / بدون قالب تأییدشده = صف ادمین.
5. ارسال سریع ماژول‌ها = یک‌بار تأیید روی خود آیتم؛ بعد از `Approved` تا ویرایش بعدی فوری ارسال می‌شود.
