...
if [[ "$DEPLOY_AFTER_LOAD" == "1" ]]; then
  ssh "$SERVER" "cd $REMOTE_API_DIR && (git pull origin main || echo 'WARN: git pull failed — continuing with uploaded image') && docker compose -f $COMPOSE_FILE --env-file $ENV_FILE up -d --no-deps --force-recreate --no-build api"
