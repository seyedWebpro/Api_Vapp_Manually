...
echo "── interpretation ──"
echo "This script does NOT call Shahkar itself — it only reads past logs."
echo "If the newest ZohalInquiryLogs row is still old (same Id/time), you have NOT"
echo "retried register from the app AFTER updating the Zohal IP allowlist."
echo ""
echo "NEXT:"
echo "  1) In Zohal dashboard click ثبت (Submit) if not already"
echo "  2) From the mobile/web app: try register again (new nationalId+mobile)"
echo "  3) Re-run: bash devops/scripts/verify-zohal.sh"
echo "  Success = a NEW row (higher Id) with OutcomeStatus != IpNotAllowed"
echo "  Still IpNotAllowed on a NEW row = allowlist not applied / wrong token / wait a few minutes"
